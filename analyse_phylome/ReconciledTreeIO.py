...
def OBSOLETEWARNINGTXT(tag):
    return (
        "The obsolete tag "
        + tag
        + " was observed and this may result in unwanted behaviour. Please use a conversion script such as convertToLossIndependentVersion.py to update your file to a newest verszion of the format."
    )
...
class recPhyloXML_parser:
    ...
    def parse(self, fileName, obsoleteTagsBehaviour=1):
        ...
        if parseFunction is None:
            raise Exception(
                "recPhyloXML exception. Problem while parsing the xml file : no recPhylo or recgeneTree tag found at the root of the file."
            )
            return None
        ...
        if obj is None:
            raise Exception(
                "recPhyloXML exception. Problem while parsing the xml file : no phylogeny or clade found?"
            )
    ...
    def parse_recPhylo(self, element, obsoleteTagsBehaviour=1):
        ...
        *recursive funtion*
        ...
        if not self.isOfTag(element, TAG):
            raise Exception(
                "BadTagException. The element is of tag "
                + element.tag
                + " instead of "
                + TAG
                + "."
    ...
    def parse_recGeneTree(self, element, obsoleteTagsBehaviour=1):
        ...
        *recursive funtion*
        ...
        for ch in children:
            if self.isOfTag(ch, "phylogeny"):
                node = self.parse_phylogeny(
                    ch, reconciled=True, obsoleteTagsBehaviour=obsoleteTagsBehaviour
                )
                break
        ...
        *recursive funtion*
            ...
            if self.isOfTag(ch, "clade"):
                if node is None:
                    ...
                    raise Exception(
                        "BadTagException. A "
                        + TAG
                        + " element has more than one clade children (only one is expected)."
                    )
            else:
        ...
        if node is None:
            raise Exception(
                "BadTagException. A "
                + TAG
                + " element has no clade children (one is expected)."
            )
            ...
            if obsoleteTagsBehaviour > 0:
                if evtCode in OBSOLETE_EVENT_TAGS:
                    ...
                    if obsoleteTagsBehaviour > 1:
                        raise Exception(
                            "ERROR. obsolete tag " + evtCode + " encoutered"
                        )

            evtCode = REVERSE_EVENTTAGCORRESPONDANCE.get(
                evtCode, evtCode
            )  ## replace by special code when known tag, otherwise keep as is
